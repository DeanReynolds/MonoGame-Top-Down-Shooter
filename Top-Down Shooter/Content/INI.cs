...
public class INI
{
    ...
    public void Set(string section, string name, string value, bool save = false) { string key = ((!string.IsNullOrEmpty(section) ? (section + ".") : string.Empty) + name); if (Nodes.Contains(key)) Nodes[key] = value; else Nodes.Add(key, value); if (save) Save(); }
